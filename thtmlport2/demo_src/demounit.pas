...
uses
  ...
  Forms, Dialogs, ExtCtrls, Menus, Htmlview, StdCtrls, Clipbrd, HTMLsubs, //ShellAPI, Unix,
  PrintersDlgs, ComCtrls;
...
{   MediaPlayer: TMediaPlayer;
    ...
    procedure ObjectClick(Sender, Obj: TObject; const AOnClickMsg: String);
...
begin
  ...
  if Viewer.TitleAttr <> '' then
    Capt := Capt+'Title: '+Viewer.TitleAttr;
  Panel1.Caption := Capt;
end;
...
begin
{$IFDEF MSWINDOWS}
  Count := DragQueryFile(AMessage.WParam, 0, @S[1], 200);
  S[0] := Char(Count);
...
procedure TForm1.ObjectClick(Sender, Obj: TObject; const AOnClickMsg: String);
var
  S: string;
begin
...
procedure TForm1.MetaRefreshEvent(Sender: TObject; Delay: Integer; const URL: String);
begin
